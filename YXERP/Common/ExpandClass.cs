...
public static class ExpandClass
{
    ...
    public const string Version = "20160625";
    ...
    public static List<Menu> GetChildMenuByCode(HttpContext httpContext, string menuCode)
    {
        ...
        {
            return ((CloudSalesEntity.Users)httpContext.Session["ClientManager"]).Menus.Where(m => m.PCode == menuCode && m.IsMenu == 1).OrderBy(m => m.Sort).ToList();
        }
        else
        {
    ...
    public static Menu GetController(HttpContext httpContext, string controller)
    {
        if (httpContext.Session["ClientManager"] != null)
        {
            return CloudSalesBusiness.CommonBusiness.ClientMenus.Where(m => m.Controller.ToUpper() == controller.ToUpper() && m.Layer == 2 && m.IsMenu == 1).FirstOrDefault();
        }
        return new Menu();
    }
    /// <summary>
