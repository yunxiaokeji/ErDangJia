...
namespace CloudSalesDAL
{
    public class ProductsDAL : BaseDAL
    {
        ...
        {
            SqlParameter[] paras = { new SqlParameter("@ClientID", clientid) };
            DataTable dt = GetDataTable("select ProviderID,Name,CMClientID,CMClientCode,ProviderType from Providers where ClientID=@ClientID and Status<>9", paras, CommandType.Text);
            return dt;
        ...
        public bool UpdateProduct(string productid, string productCode, string productName, string generalName, bool iscombineproduct, string providerid, string brandid, string bigunitid, string UnitID, int bigSmallMultiple,
                            int status, string categoryid, string attrlist, string valuelist, string attrvaluelist, string attrvalueStr, decimal commonprice, decimal price,
                            decimal weight, bool isnew, bool isRecommend, int isallow, int isautosend, int effectiveDays, decimal discountValue, int warnCount, 
