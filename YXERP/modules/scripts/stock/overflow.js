...
define(function (require, exports, module) {
    ...
    ObjectJS.bindEvent = function (wares) {
        ...
        $("#delete").click(function () {
            ...
            confirm("报溢单删除后不可恢复,确认删除吗？", function () {
                Global.post("/Stock/DeleteOverflowDoc", { docid: _this.data("id") }, function (data) {
                    if (data.status) {
                        alert("删除成功");
                    } else {
                        alert("删除失败");
        ...
        $("#printOrderOut").click(function () {
            ...
            if (checks.length == 1) {
                ...
                if (checks.length == 0) {
                    alert("您尚未选择要打印的库存报溢单");
                } else {
                    alert("目前只支持单条打印的库存报溢单");
                }
            }
        });
    }
    ...
    ObjectJS.dropdownul = function () {
        $(".dropdown").click(function () {
            ...
            $("#auditDropdown li").data("id", _this.data("id")).data("url", "/Stock/OverflowDetail/" + _this.data("id"));
        });
    }
