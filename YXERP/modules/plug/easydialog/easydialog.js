...
define(function (require, exports, module) {
    ...
    var easyDialog = function () {
         ...
         */
        Dialog.drag = function (guid, target, moveElem) {
            // 清除文本选择
            var clearSelect = 'getSelection' in win ? function () {
                ...
                _fixed = Dialog.data(guid + 'options').fixed;
        ...
        var extend = {
            open: function () {
                ...
                // 如果页面中已经缓存遮罩层，直接显示
                if (options.overlay) {
                    overlay = doc.getElementById(options.guid + 'overlay');
                    if (!overlay) {
                        overlay = $.createOverlay(options.guid);
                ...
                if (options.autoClose && typeof options.autoClose === 'number') {
                    timer = setTimeout(self.close(options.guid), options.autoClose);
                }
                // 绑定拖拽(如果弹出层内容的宽度或高度溢出将不绑定拖拽)
                if (options.drag && dialogTitle && !widthOverflow && !heigthOverflow) {
                    dialogTitle.style.cursor = 'move';
                    Dialog.drag(options.guid, dialogTitle, dialogBox);
                }
                ...
                Dialog.data(options.guid + 'dialogElements', {
                    overlay: overlay,
                    dialogBox: dialogBox,
                    closeBtn: closeBtn,
            ...
            close: function (guid) {
                ...
                if (options) {
                    ...
                    Dialog.removeData(guid + 'options');
                }
                // 清除缓存
